...
public:
    ...
    void analyzeConvergence(double t) {
        ...
        double error_coarse = abs(computeExactSolution(t) - computeNumericalSolution(n, step));
        double error_fine = abs(computeExactSolution(t) - computeNumericalSolution(2*n, step/2));
        double convergence_order = log2(error_coarse / error_fine);
