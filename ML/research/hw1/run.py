...
def run_single_test(data_dir, output_dir):
    ...
    if os.path.exists(os.path.join(data_dir, 'task1_vectorised')): 
        for i in range(F1_TESTS_NUM):
            ...
            tick = time.time()
            res = research_functions_vectorised.are_multisets_equal(x, y)
        ...
        for i in range(F1_TESTS_NUM):
            x = np.load(os.path.join(data_dir, f'input_{i}', 'x.npy'))
            y = np.load(os.path.join(data_dir, f'input_{i}', 'y.npy'))
            res = research_functions.are_multisets_equal(x.tolist(), y.tolist())
            data = {'res': res}
        ...
        for i in range(F2_TESTS_NUM):
            x = np.load(os.path.join(data_dir, f'input_{i}', 'x.npy'))
            tick = time.time()
            res = research_functions_vectorised.max_prod_mod_3(x)
            res_time = time.time() - tick
            data = {'res': res, 'time': res_time}
        ...
        for i in range(F2_TESTS_NUM):
            x = np.load(os.path.join(data_dir, f'input_{i}', 'x.npy'))
            res = research_functions.max_prod_mod_3(x.tolist())
            data = {'res': res}
        ...
        for i in range(F3_TESTS_NUM):
            ...
            tick = time.time()
            res = research_functions_vectorised.convert_image(image, weights)
            res_time = time.time() - tick
            data = {'res': res, 'time': res_time}
        ...
        for i in range(F3_TESTS_NUM):
            image = np.load(os.path.join(data_dir, f'input_{i}', 'image.npy'))
            weights = np.load(os.path.join(data_dir, f'input_{i}', 'weights.npy'))
            res = research_functions.convert_image(image.tolist(), weights.tolist())
            data = {'res': res}
    ...
    if os.path.exists(os.path.join(data_dir, 'task4_vectorised')): 
        for i in range(F4_TESTS_NUM):
            x = np.load(os.path.join(data_dir, f'input_{i}', 'x.npy'))
            y = np.load(os.path.join(data_dir, f'input_{i}', 'y.npy'))
            tick = time.time()
            res = research_functions_vectorised.rle_scalar(x, y)
            res_time = time.time() - tick
            data = {'res': res, 'time': res_time}
        ...
        for i in range(F4_TESTS_NUM):
            x = np.load(os.path.join(data_dir, f'input_{i}', 'x.npy'))
            y = np.load(os.path.join(data_dir, f'input_{i}', 'y.npy'))
            res = research_functions.rle_scalar(x.tolist(), y.tolist())
            data = {'res': res}
    ...
    elif os.path.exists(os.path.join(data_dir, 'task5_vectorised')): 
        for i in range(F5_TESTS_NUM):
            x = np.load(os.path.join(data_dir, f'input_{i}', 'x.npy'))
            y = np.load(os.path.join(data_dir, f'input_{i}', 'y.npy'))
            tick = time.time()
            res = research_functions_vectorised.cosine_distance(x, y)
            res_time = time.time() - tick
            data = {'res': res, 'time': res_time}
        ...
        for i in range(F5_TESTS_NUM):
            x = np.load(os.path.join(data_dir, f'input_{i}', 'x.npy'))
            y = np.load(os.path.join(data_dir, f'input_{i}', 'y.npy'))
            res = research_functions.cosine_distance(x.tolist(), y.tolist())
            data = {'res': res}
    ...
    if os.path.exists(os.path.join(output_dir, 'task1_vectorised')):
        for i in range(F1_TESTS_NUM):
            ...
            if type(data['res']) not in (bool, np.bool_):
                res = f'research_functions_vectorised.py, are_multisets_equal: Wrong answer type'
                print(res)               
                return res

            # check answer
            if data['res'] != gt_data['res']:
                res = f'research_functions_vectorised.py, are_multisets_equal: Wrong answer on test {i}'
                print(res)
                return res
            ...
            if i == F1_TESTS_NUM - 1:
                if data['time'] > tol * gt_data['time']:
                    res = f"research_functions_vectorised.py, are_multisets_equal: Your solution is too slow! Execution time on test {i} is {data['time']:.4f} sec"
                    print(res)
                    return res

        res = 'Ok'
        print('research_functions_vectorised.py, are_multisets_equal: Ok')  
        ...
        for i in range(F1_TESTS_NUM):
            ...
            # check type
            if type(data['res']) not in (bool, np.bool_):
                res = f'research_functions.py, are_multisets_equal: Wrong answer type'
                print(res)               
                return res

            # check answer
            if data['res'] != gt_data['res']:
                res = f'research_functions.py, are_multisets_equal: Wrong answer on test {i}'
                print(res)
                return res

        res = 'Ok'
        print('research_functions.py, are_multisets_equal: Ok')         
        ...
        for i in range(F2_TESTS_NUM):
            ...
            if type(data['res']) not in (int, np.integer, np.int32, np.int64):
                res = f'research_functions_vectorised.py, max_prod_mod_3: Wrong answer type'
                print(res)               
                return res

            # check answer
            if data['res'] != gt_data['res']:
                res = f'research_functions_vectorised.py, max_prod_mod_3: Wrong answer on test {i}'
                print(res)
                return res
            ...
            if i == F2_TESTS_NUM - 1:
                if data['time'] > tol * gt_data['time']:
                    res = f"research_functions_vectorised.py, max_prod_mod_3: Your solution is too slow! Execution time on test {i} is {data['time']:.4f} sec"
                    print(res)
                    return res      

        res = 'Ok'
        print('research_functions_vectorised.py, max_prod_mod_3: Ok')
        ...
        for i in range(F2_TESTS_NUM):
            ...
            # check type
            if type(data['res']) not in (int, np.integer, np.int32, np.int64):
                res = f'research_functions.py, max_prod_mod_3: Wrong answer type'
                print(res)               
                return res

            # check answer
            if data['res'] != gt_data['res']:
                res = f'research_functions.py, max_prod_mod_3: Wrong answer on test {i}'
                print(res)
                return res
           

        res = 'Ok'
        print('research_functions.py, max_prod_mod_3: Ok')
        ...
        for i in range(F3_TESTS_NUM):
            ...
            if type(data['res']) != np.ndarray:
                res = f'research_functions_vectorised.py, convert_image: Wrong answer type'
                print(res)               
                return res
            ...
            if ((data['res'].shape != gt_data['res'].shape) or 
                ...
                res = f'research_functions_vectorised.py, convert_image: Wrong answer on test {i}'
                print(res)
                return res
            ...
            if i == F3_TESTS_NUM - 1:
                if data['time'] > tol * gt_data['time']:
                    res = f"research_functions_vectorised.py, convert_image: Your solution is too slow! Execution time on test {i} is {data['time']:.4f} sec"
                    print(res)
                    return res      

        res = 'Ok'
        print('research_functions_vectorised.py, convert_image: Ok')   
        ...
        for i in range(F3_TESTS_NUM):
            ...
            if type(data['res']) != list:
                res = f'research_functions.py, convert_image: Wrong answer type'
                print(res)               
                return res
            ...
            if ((data['res'].shape != gt_data['res'].shape) or 
                (not np.allclose(data['res'], gt_data['res']))):
                res = f'research_functions.py, convert_image: Wrong answer on test {i}'
                print(res)
                return res

        res = 'Ok'
        print('research_functions.py, convert_image: Ok')    
        ...
        for i in range(F4_TESTS_NUM):
            ...
            # check type
            if type(data['res']) not in (int, np.integer, np.int32, np.int64):
                res = f'research_functions_vectorised.py, rle_scalar: Wrong answer type'
                print(res)               
                return res

            # check answer
            if data['res'] != gt_data['res']:
                res = f'research_functions_vectorised.py, rle_scalar: Wrong answer on test {i}'
                print(res)
                return res
            ...
            if i == F4_TESTS_NUM - 1:
                if data['time'] > tol * gt_data['time']:
                    res = f"research_functions_vectorised.py, rle_scalar: Your solution is too slow! Execution time on test {i} is {data['time']:.4f} sec"
                    print(res)
                    return res      

        res = 'Ok'
        print('research_functions_vectorised.py, rle_scalar: Ok')   
        ...
        for i in range(F4_TESTS_NUM):
            ...
            with open(os.path.join(gt_dir, f'output_{i}.pkl'), 'rb') as f:
             ...
             # check type
            if type(data['res']) not in (int, np.integer, np.int32, np.int64):
                res = f'research_functions.py, rle_scalar: Wrong answer type'
                print(res)               
                return res

            # check answer
            if data['res'] != gt_data['res']:
                res = f'research_functions.py, rle_scalar: Wrong answer on test {i}'
                print(res)
                return res

        res = 'Ok'
        print('research_functions.py, rle_scalar: Ok')   
        ...
        for i in range(F5_TESTS_NUM):
            ...
            # check type
            if type(data['res']) != np.ndarray:
                res = f'research_functions_vectorised.py, cosine_distance: Wrong answer type'
                print(res)               
                return res
            ...
            if ((data['res'].shape != gt_data['res'].shape) or 
                (not np.allclose(data['res'], gt_data['res']))):
                res = f'research_functions_vectorised.py, cosine_distance: Wrong answer on test {i}'
                print(res)
                return res
            ...
            if i == F5_TESTS_NUM - 1:
                if data['time'] > tol * gt_data['time']:
                    res = f"research_functions_vectorised.py, cosine_distance: Your solution is too slow! Execution time on test {i} is {data['time']:.4f} sec"
                    print(res)
                    return res      

        res = 'Ok'
        print('research_functions_vectorised.py, cosine_distance: Ok')  
        ...
        for i in range(F5_TESTS_NUM):
            ...
            # check type
            if type(data['res']) != list:
                res = f'research_functions.py, cosine_distance: Wrong answer type'
                print(res)               
                return res
            ...
            if ((data['res'].shape != gt_data['res'].shape) or 
                (not np.allclose(data['res'], gt_data['res']))):
                res = f'research_functions.py, cosine_distance: Wrong answer on test {i}'
                print(res)
                return res 

        res = 'Ok'
        print('research_functions.py, cosine_distance: Ok') 
